...
SELECT Task.task_id, Task.name AS task_name, Task.due_date,
	COALESCE(Task.description, 'No description provided.') AS task_description, COALESCE(Task.budgeted_hours,0) AS time_budgeted, Task.task_status_id,
	(CASE WHEN Task.task_status_id=3 /* QA */ THEN REF_Task_Status.description || ' by ' || QA.last_name ELSE REF_Task_Status.status END) AS task_status, Owner.last_name AS task_owner,
...
WHERE Project.project_id=#attributes.project_id#
	AND Task.task_status_id!=7 /*exclude closed tasks*/<cfif isdefined("session.workstream_task_list_order")>
