...
FROM (
	...
	FROM Time_Entry
		LEFT OUTER JOIN Task ON Time_Entry.Task_ID = Task.Task_ID RIGHT OUTER JOIN
          Marketing
	INNER JOIN       Project ON       Marketing.project_id = Project.project_id
	LEFT OUTER JOIN        Marketing_Emp
	INNER JOIN       Emp_Contact ON       Marketing_Emp.emp_id = Emp_Contact.emp_id ON       Project.project_id = Marketing_Emp.project_id LEFT OUTER
           JOIN
          Emp_Contact Emp_Contact1 ON       Marketing.Source = Emp_Contact1.emp_id ON       Task.project_id = Project.project_id
