...
<cfsilent>
 ...
 --->
<!--- $issue$: can this be consolidated with qry_get_time_entry_edit.cfm? Combine as qry_get_time_entry.cfm? --->
...
SELECT Notes.notes_type_id, (Demographics.last_name || ', ' || Demographics.first_name) AS author, (LEFT(Demographics.first_name,1) || LEFT(Demographics.last_name,1)) AS initials,
	COALESCE(Time_Entry.work_date, Notes.created_date) AS work_date, COALESCE(Time_Entry.time_entry_id, 0) AS time_entry_id, Notes.note
FROM Notes
