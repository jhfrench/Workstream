...
SELECT Time_Entry.time_entry_id, Hours_Pin_Week.sumhoursweek,
	...
	(CAST(REF_Day_Of_Week.day_name AS varchar(9)) || ', ' || CAST(DATEPART(month,Time_Entry.date) AS varchar(2)) || '/' || CAST(DATEPART(day,Time_Entry.date) AS varchar(11)) || '/' || CAST (CAST(RIGHT(EXTRACT(YEAR FROM Time_Entry.date),2) AS varchar(11)) AS varchar(11)) || '- ' || CAST(Hours_Pin_Date.sumhours AS varchar(11))) AS workdays, 
