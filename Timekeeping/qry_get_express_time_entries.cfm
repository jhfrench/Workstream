...
FROM Time_Entry, REF_Day_of_Week, Project, Notes,
	(
		SELECT Time_Entry.work_date, SUM(Time_Entry.Hours) AS sumhours, Time_Entry.emp_id
		FROM Time_Entry 
	...
	(
		SELECT EXTRACT(YEAR FROM Time_Entry.work_date) AS year, DATEPART(ww, Time_Entry.work_date) AS week, SUM(Time_Entry.hours) AS sumhoursweek, 
...
WHERE Time_Entry.active_ind=1
	...
	AND EXTRACT(YEAR FROM Time_Entry.work_date)=Hours_Pin_Week.year
