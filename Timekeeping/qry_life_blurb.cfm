...
FROM
	...
	FROM Demographics_Ngauge AS Demographics, Company
	WHERE Demographics.emp_id=Company.emp_id
		...
		AND Company.company=#listlast(session.workstream_company_id)#
	...
	FROM Demographics_Ngauge AS Demographics, Company, Emp_Contact
	WHERE Demographics.emp_id=Emp_Contact.emp_id
		AND Demographics.emp_id=Company.emp_id
		AND (Demographics.effective_to IS NULL OR Demographics.effective_to > GETDATE())
	...
	FROM Demographics_Ngauge AS Demographics, Company, Security
	WHERE Demographics.emp_id=Company.emp_id
