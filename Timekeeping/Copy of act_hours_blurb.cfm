...
<cfsilent>
	...
	Edits:
	$Log$
	Revision 1.1  2005/03/09 18:20:36  stetzer
	<>
...
<cfset should_have=DateDiff("d","#Month(now())#/1/#Year(now())#",now())*5.2>
</cfsilent>
<cfinclude template="qry_hours_blurb.cfm">
<cfinclude template="qry_time_allocation_blurb.cfm">
<cfoutput query="hours_blurb">
	<cfset transaction_date=month("#work_month#/13/#work_year#")>
	<cfset total=month_hours+total>
	<cfif transaction_date EQ month(dateadd("m",-1,now()))>
		<cfset last_month=month_hours>
	<cfelse>
		<cfset this_month=month_hours>
	</cfif>
	<cfif len(last_month) and last_month NEQ 0>
		<cfset difference=numberformat((this_month-last_month)/last_month*100)>
	</cfif>
</cfoutput>
<cfset hours_blurb="According to your time entries, this month you worked #DecimalFormat(this_month)# hours. Last month you worked #DecimalFormat(last_month)# hours.">
<cfif difference GT 0>
	<cfset hours_blurb="#hours_blurb# This is an increase of #difference#% from last month.">
<cfelseif difference LT 0>
	<cfset difference=difference*-1>
	<cfset hours_blurb="#hours_blurb# This is a decrease of #difference#% from last month.">
</cfif>
...
<cfif this_month LT should_have>
...
<cfset variables.work_allocation='
						<applet code="javachart.applet.spinningPie.class" archive="javachart/jars/pie.jar, javachart/jars/base.jar" codebase="#request.dir_level##application.graphbase#" width="300" height="150">
							...
							<param name="labelFont" value="Trebuchet MS,#small#,1">
