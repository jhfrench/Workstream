...
<cfset StatusMsg=ReplaceList(task_name, variables.StatusMsg_replace, variables.StatusMsg_replace_with)>
	<tr<cfif (currentrow MOD 2)> bgcolor="##E1E1E1"</cfif>>
		...
		<td class="RegText#session.workstream_text_size#"><a href="javascript:list_to_time('#task_id#');" onmouseover="MM_displayStatusMsg('View time details for #StatusMsg#.');return document.MM_returnValue;" onmouseout="MM_displayStatusMsg('');" class="RegText#session.workstream_text_size#"><cfif listlen(time_used) GT 1 AND listgetat(time_used,2,".") GT 0>#decimalformat(time_used)#<cfelse>#numberformat(time_used)#</cfif><cfif time_budgeted>/#time_budgeted# #numberformat(percent_time_used)#%</cfif></a></td>
		...
		<td class="RegText#session.workstream_text_size#"><cfif NOT now() LT date_due><img src="#request.dir_level##application.application_specific_settings.image_dir#<cfif dateformat(now(), "mm/dd/yyyy") EQ dateformat(date_due, "mm/dd/yyyy")>not_started<cfelse>overdue</cfif>.gif" width="17" height="17" alt="" border="0">&nbsp;</cfif>#task_status#</td>
	</tr>
</cfoutput>
</cfoutput>
<cfelse>
