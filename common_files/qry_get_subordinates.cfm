...
WHERE Emp_Contact.emp_id=Link_Employee_Supervisor.user_account_id
	...
	AND <cfif len(attributes.date_linked)>#createodbcdate(attributes.date_linked)# BETWEEN Link_Employee_Supervisor.date_start AND ISNULL(Link_Employee_Supervisor.date_end,DATEADD(D,1,#createodbcdate(attributes.date_linked)#))<cfelse>Link_Employee_Supervisor.active_ind=1</cfif>
	AND Security.disable=0
	AND GETDATE() BETWEEN Demographics.effective_from AND ISNULL(Demographics.effective_to,DATEADD(D,1,GETDATE()))</cfif><cfif NOT isdefined("attributes.hide_supervisor")> OR Link_Employee_Supervisor.user_account_id=#session.user_account_id#</cfif>)
