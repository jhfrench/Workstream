...
<cfif isdefined("attributes.date_linked")>/*we're looking for the organization at a specified point in time*/
	AND <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.date_linked#" /> BETWEEN Link_User_Account_Supervisor.date_start AND COALESCE(Link_User_Account_Supervisor.date_end, <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.date_linked#" />)
	AND <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.date_linked#" /> BETWEEN Employee.hire_date AND COALESCE(Employee.turnover_date, <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.date_linked#" />)
<cfelseif isdefined("attributes.start_date") AND isdefined("attributes.end_date")>/*this is the goal-post scenario*/
	AND Link_User_Account_Supervisor.date_start < <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.end_date#" />
	AND COALESCE(Link_User_Account_Supervisor.date_end, <cfqueryparam cfsqltype="cf_sql_date" value="#dateadd("d", 1, attributes.start_date)#" />) > <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.start_date#" />
	AND Employee.hire_date < <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.end_date#" />
	AND COALESCE(Employee.turnover_date, <cfqueryparam cfsqltype="cf_sql_date" value="#dateadd("d", 1, attributes.start_date)#" />) > <cfqueryparam cfsqltype="cf_sql_date" value="#attributes.start_date#" />
