...
<fusedoc language="ColdFusion MX" specification="2.0" template="qry_get_nsm_hierarchy_orgs_from_id.cfm">
	...
	<properties>
		<history email="kpoon@hq.nasa.gov" author="Kelly Poon" type="create" date="3/28/2008" role="FuseCoder" comments="Created File">
			$Id:$
		</history>
	</properties>
	<IO>
...
SELECT Hierarchy_Assignment.organization_id, ISNULL(Hierarchy_Assignment.parent_organization_id,0) AS parent_organization_id, REF_Organization.description + ' (' + REF_Organization.organization_code + ')' AS nsm_organization_description, 
...
WHERE Hierarchy_Assignment.active_ind=1
	...
	<cfif isdefined("attributes.organization_id")>
		AND ISNULL(Hierarchy_Assignment.parent_organization_id,0)=#attributes.organization_id#
	</cfif>
