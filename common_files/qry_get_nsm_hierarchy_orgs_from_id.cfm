...
SELECT Hierarchy_Assignment.organization_id, NVL(Hierarchy_Assignment.parent_organization_id,0) AS parent_organization_id, REF_Organization.description || ' (' || REF_Organization.organization_code || ')' AS nsm_organization_description, 
...
WHERE Hierarchy_Assignment.active_ind=1
	...
	<cfif isdefined("attributes.organization_id")>
		AND NVL(Hierarchy_Assignment.parent_organization_id,0)=#attributes.organization_id#
	</cfif>
