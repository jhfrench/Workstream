...
<fusedoc language="ColdFusion MX" specification="2.0" template="qry_get_nsm_children_from_id.cfm">
	...
	<properties>
		<history email="jeromy_french@hotmail.com" author="Jeromy French" type="create" date="3/30/2008" role="FuseCoder" comments="Created File">
			$Id:$
		</history>
	</properties>
	<IO>
...
<!--- $issue$: convert from Oracle-specific START WITH/CONNECT BY to Postgres recursive query --->
	SELECT Hierarchy_Assignment.organization_id, COALESCE(Hierarchy_Assignment.parent_organization_id,0) AS parent_organization_id, Link_Program_Year_Hierarchy.hierarchy_level_id,
	...
	FROM Hierarchy_Assignment
		...
		INNER JOIN REF_Organization ON Hierarchy_Assignment.organization_id=REF_Organization.organization_id
 			...
 			START WITH COALESCE(Hierarchy_Assignment.parent_organization_id,0)= #attributes.delete_organization_id#
 			CONNECT BY PRIOR Hierarchy_Assignment.organization_id=Hierarchy_Assignment.parent_organization_id
	) a
WHERE 1=1<cfif isdefined("attributes.lowest_hierarchy_level_delete")>
	AND a.hierarchy_level_id=#attributes.lowest_hierarchy_level_delete#</cfif>
