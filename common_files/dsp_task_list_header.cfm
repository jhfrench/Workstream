...
<cfoutput>
	<tr>
		<th class="SubHeadText" nowrap>
			<a href="javascript:order_by('task_id');" onmouseover="MM_displayStatusMsg('ORDER BY the task ID.');return document.MM_returnValue;" onmouseout="MM_displayStatusMsg('');" class="SubHeadText"><abbr title="task id">ID</abbr> <cfif NOT compare(ListFirst(ListFirst(session.workstream_task_list_order), " "),"task_id")><img src="#request.dir_level##application.application_specific_settings.image_dir#order_<cfif ListLen(ListFirst(session.workstream_task_list_order), " ") EQ 2>down<cfelse>up</cfif>.gif" width="8" height="8" alt="ORDER BY the task ID." border="0"></cfif></a>
		</th>
		<th class="SubHeadText" nowrap>
