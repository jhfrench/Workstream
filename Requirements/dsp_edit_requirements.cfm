			...
			<tr>
				...
				<td>
					...
					<br /><cfselect name="product_version_id" id="requirement_type_id" required="yes" message="Please specify the type of requirement you are entering.">
							...
							<option value="#get_ref_product_version.product_version_id#"<cfif variables.product_version_id EQ get_ref_product_version.product_version_id> selected="selected"</cfif>>#description# (anticipated release:<cfif len(scheduled_release_date)>#dateformat(scheduled_release_date, "m/d/yyyy")#<cfelse>unknown</cfif>)</option>
							</cfloop>
						</cfselect>
				</td>
			</tr>
