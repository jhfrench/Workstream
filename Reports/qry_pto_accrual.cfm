...
COALESCE((CASE WHEN EXTRACT(MONTH FROM tenure_date) < EXTRACT(MONTH FROM CURRENT_DATE) then
	...
	cast(cast(month(tenure_date) AS VARCHAR(2)) || '/01/' || CAST(EXTRACT(YEAR FROM CURRENT_DATE) AS VARCHAR(4)) as smalldatetime)) )
...
(CASE WHEN EXTRACT(MONTH FROM tenure_date) >= EXTRACT(MONTH FROM CURRENT_DATE) THEN 0 else EXTRACT(MONTH FROM CURRENT_DATE)- month(tenure_date) end) *
/*<!--- IF the MONTH of your hire is less than this month then go select your new accrual rate... in the sub query. --->*/
(CASE WHEN EXTRACT(MONTH FROM tenure_date) < EXTRACT(MONTH FROM CURRENT_DATE) 
	then
