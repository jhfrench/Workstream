...
<cfform name="individual_hours_detail" action="index.cfm?fuseaction=#attributes.fuseaction#" method="post" class="well form-inline">
	<fieldset>
		...
		<input type="date" name="from_date" id="from_date" min="#dateformat(application.application_specific_settings.workstream_start_date, 'yyyy-mm-dd')#" maxlength="10" required="required" value="#dateformat(attributes.from_date, 'yyyy-mm-dd')#" class="span3 date" />
		<label for="through_date">To</label>
		<input type="date" name="through_date" id="through_date" min="#dateformat(application.application_specific_settings.workstream_start_date, 'yyyy-mm-dd')#" maxlength="10" required="required" value="#dateformat(attributes.through_date, 'yyyy-mm-dd')#" class="span3 date" />
