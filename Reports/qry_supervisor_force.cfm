...
<cfsilent>
	...
	Edits:
	$Log$
Revision 1.1  2005/03/09 18:14:46  stetzer
<>
...
FROM Demographics, Emp_Contact LEFT OUTER JOIN 
...
FROM Time_Entry 
	RIGHT OUTER JOIN Forecast_assignment 
	ON Time_Entry.task_id=Forecast_assignment.task_id, 
	Task, Team
WHERE Task.task_id=Forecast_assignment.task_id
	...
	AND Team.roll_id=1
GROUP BY Forecast_assignment.emp_id) AS bdata 
ON bdata.emp_id=emp_contact.emp_id  LEFT OUTER JOIN 
	...
	Time_Entry.emp_id
	FROM Time_Entry 
		LEFT OUTER JOIN Forecast_assignment 
		ON Time_Entry.task_id=Forecast_assignment.task_id, Task, Team
	WHERE Task.task_id=Time_Entry.task_id
		AND Task.due_date BETWEEN #createodbcdatetime(attributes.from_date)# AND #createodbcdatetime(attributes.to_date)#
		AND Time_Entry.emp_id=Team.emp_id
		AND Time_Entry.task_id=Team.task_id
		AND Team.roll_id=1
GROUP BY Time_Entry.emp_id) AS nbdata ON nbdata.emp_id=emp_contact.emp_id 
