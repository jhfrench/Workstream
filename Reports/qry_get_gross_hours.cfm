...
SELECT Demographics.user_account_id, (Demographics.last_name || ', ' || Demographics.first_name) AS name,<cfloop from="1" to="#variables.daysinmonth#" index="ii">
...
FROM Demographics
	...
	INNER JOIN Employee ON Demographics.user_account_id=Employee.user_account_id
		AND Employee.hire_date <= #variables.date_closed#
		AND COALESCE(Employee.turnover_date,#variables.date_open#) >= #variables.date_open#
...
WHERE Demographics.active_ind=1<cfif isdefined("attributes.user_account_id") AND comparenocase(attributes.user_account_id, "ALL")>
