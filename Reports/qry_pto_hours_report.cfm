...
FROM Emp_Contact
	INNER JOIN PTO_Hours ON Emp_Contact.user_account_id=PTO_Hours.user_account_id
	INNER JOIN Link_Company_Emp_Contact ON Emp_Contact.user_account_id=Link_Company_Emp_Contact.user_account_id
	INNER JOIN View_Demographics_Workstream Demographics ON Emp_Contact.user_account_id=Demographics.user_account_id
...
WHERE Link_Company_Emp_Contact.company_id IN (
	...
	AND Emp_Contact.user_account_id IN ('#preservesinglequotes(attributes.drill_down)#')
ORDER BY lname
<cfelse>
SELECT DISTINCT Emp_Contact.user_account_id, Emp_Contact.name, Emp_Contact.lname,
	Used_Hours.used_hours, PTO_Hours.pto_type_indicator
FROM Security_Company_Access
	INNER JOIN Emp_Contact ON Security_Company_Access.user_account_id=Emp_Contact.user_account_id
	INNER JOIN PTO_Hours ON Emp_Contact.user_account_id=PTO_Hours.user_account_id
	INNER JOIN View_Demographics_Workstream Demographics ON Emp_Contact.user_account_id=Demographics.user_account_id
	...
	INNER JOIN (
		SELECT Link_Company_Emp_Contact.user_account_id, COALESCE(SUM(Time_Entry.hours),0) AS used_hours
		FROM Link_Company_Emp_Contact
			LEFT OUTER JOIN Time_Entry ON Link_Company_Emp_Contact.user_account_id=Time_Entry.user_account_id
		...
		GROUP BY Link_Company_Emp_Contact.user_account_id
	) AS Used_Hours ON Emp_Contact.user_account_id=Used_Hours.user_account_id
