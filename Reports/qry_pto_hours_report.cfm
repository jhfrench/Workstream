...
ORDER BY lname
<cfelse>
SELECT DISTINCT Emp_Contact.Name AS name, Emp_Contact.LName AS lname, Emp_Contact.emp_id AS pin<!--- $issue$ how is this used? can it be converted to emp_id or user_account_id? --->,
...
FROM Security_Company_Access, Emp_Contact, PTO_Hours, Demographics, REF_Day_Length, security,
	(SELECT COALESCE(SUM(Time_Entry.hours),0) AS used_hours, Link_Company_Emp_Contact.emp_id
