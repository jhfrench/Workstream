...
FROM Emp_Contact,
	(SELECT IT_Hours.emp_id, ISNULL(IT_Hours.it_hours,0) AS it_hours, ISNULL(((IT_Hours.it_hours/Ttl_hours.ttl_hours)*100),0) AS it_time_percent
	FROM
		(SELECT Emp_Contact.emp_id, ISNULL(SUM(hours),0) AS it_hours
