...
SELECT Emp_Contact.user_account_id, Emp_Contact.lname, Emp_Contact.name, Efficiency_Percent.efficiency_hours, Efficiency_Percent.efficiency_percent
FROM Emp_Contact
	INNER JOIN (
		...
		FROM (
				SELECT Emp_Contact.user_account_id, COALESCE(SUM(hours),0) AS efficiency_hours
				FROM Emp_Contact
				...
				WHERE Emp_Contact.user_account_id IN (#valuelist(get_subordinates.user_account_id)#)
	...
	) AS Efficiency_Percent ON Emp_Contact.user_account_id=Efficiency_Percent.user_account_id
