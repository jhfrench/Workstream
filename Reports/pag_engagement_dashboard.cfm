...
<form name="form_engagement_dashboard" action="index.cfm?fuseaction=<cfoutput>#attributes.fuseaction#</cfoutput>" method="post" class="well form-inline">
			...
			<select name="project_manager_id" class="span2">
				...
				<cfoutput query="get_engagement_ie">
					<option value ="#user_account_id#"<cfif attributes.project_manager_id EQ user_account_id> selected="selected"</cfif>>#lname#</option>
				</cfoutput>
			</select>
...
<cfmodule template="../common_files/act_drilldown_form.cfm" function_name="edit_project" fuseaction="Customers.edit_engagement" field_name="project_id" processform="true" option="1" engagement_dashboard_return="1" customer_id="#attributes.customer_id#" project_manager_id="#attributes.project_manager_id#" sort="#attributes.sort#">
