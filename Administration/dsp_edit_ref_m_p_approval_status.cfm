...
<form name="edit_ref_m_p_approval_status_form" action="index.cfm?fuseaction=#fuseaction#" method="post">
	...
	<cfloop query="get_ref_m_p_approval_status"><option value="#m_p_approval_status_id#"<cfif NOT comparenocase(get_ref_m_p_approval_status.m_p_approval_status_id, attributes.m_p_approval_status_id)> selected="selected"</cfif>>#description#</option></cfloop>
	</select>
		...
		<table width="100%" cellspacing="0" cellpadding="8" border="0" summary="table displays user account information">
			<tr>
				...
				<td>
					...
					<br />
					<input type="number" name="sort_order" id="sort_order" value="#IIf(attributes.m_p_approval_status_id EQ 0, ('get_ref_m_p_approval_status.recordcount+1'), ('get_ref_m_p_approval_status.sort_order'))#" step="1" min="1" required="required" class="span5" />
				</td>
