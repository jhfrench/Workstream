<!-- Administration/qry_get_user_navigation_access.cfm
...
SELECT REF_Module.module_id, REF_Module.description AS module_description, REF_Business_Function.business_function_id,
	...
	CASE WHEN Access_User_Business_Function.business_function_id IS NOT NULL THEN ' checked="checked"' END AS checked_ind,
	COALESCE(Access_User_Business_Function.business_function_id, 0) AS old_business_function_id
FROM REF_Module
	...
	LEFT OUTER JOIN (
		...
		WHERE Access_User_Business_Function.active_ind=1
			AND Access_User_Business_Function.program_year_id=<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.program_year_id#">
			AND Access_User_Business_Function.user_account_id IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.user_account_id#" list="yes">)
