...
SELECT Emp_Contact.user_account_id, Emp_Contact.lname, Emp_Contact.name,
	 Link_Company_User_Account.company_id, COALESCE(REF_Company.description,'NA') AS company_name Email.email
FROM Emp_Contact
...
ORDER BY Link_Company_User_Account.company_id, lname, name, email_type_id
</cfquery>
</cfsilent>
