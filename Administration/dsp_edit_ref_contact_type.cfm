...
<form name="edit_ref_contact_type_form" action="index.cfm?fuseaction=#fuseaction#" method="post">
	Retrieve an existing <label for="contact_type_id">contact type</label>:<br />
	<select name="contact_type_id" id="contact_type_id">
	<cfloop query="get_ref_contact_type"><option value="#contact_type_id#" <cfif not comparenocase(get_ref_contact_type.contact_type_id, attributes.contact_type_id)>selected</cfif>>#description#</option></cfloop>
	</select><br />
	<input type="submit" name="method" value="Retrieve and edit Contact Type" />
</form>
