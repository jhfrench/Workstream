...
<cftry>
	...
	<cfcatch type="Database">
		...
		<cfif NOT comparenocase(cfcatch.message,"ERROR: One or more employees are missing a billing rate.")>
			<strong>Uh oh</strong>
			<p>#cfcatch.message#</p>
			<p>Please <a href="index.cfm?fuseaction=Tools.rate_change">create the necessary billing rates</a> for #get_customer_invoice_details.customer_name#.</p>
...
<cfif variables.continue_processing_ind>
	...
	SELECT project_id, project_code, project_name,
		SUM(revenue) AS project_revenue, COUNT(*) AS entry_count
