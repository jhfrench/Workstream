...
SELECT Hours_ID.project_id, Hours_ID.hours, COALESCE(Billing_Rate.rate,0) AS rate,
	(Hours_ID.hours*COALESCE(Billing_Rate.rate,0)) AS bill, Hours_ID.user_account_id, (Emp_Contact.lname || ', ' || Emp_Contact.name) AS name, 
...
FROM (
	...
	INNER JOIN Emp_Contact ON Hours_ID.user_account_id=Emp_Contact.user_account_id
	INNER JOIN View_Demographics_Workstream Demographics ON Hours_ID.user_account_id=Demographics.user_account_id
...
GROUP BY Hours_ID.project_id, Project.billable_type_id, Hours_ID.hours, 
	Billing_Rate.rate, Hours_ID.user_account_id, Emp_Contact.name, 
