...
SELECT 1 AS major_sort_order, Invoice.invoice_id, Invoice.created_date,
	...
	Demographics.last_name||', '||Demographics.first_name AS invoicer, SUM(Billing_History.total_bill_amount) AS invoice_bill_amount, SUM(COALESCE(Billing_History.payment_received_amount,0)) AS invoice_received_amount
FROM Invoice
