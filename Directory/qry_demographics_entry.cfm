...
VALUES ('#attributes.lname#', #variables.emp_id#, '#attributes.ssn#',
	'#attributes.pin#', #createodbcdatetime(attributes.hire_date)#, #createodbcdatetime(attributes.dob)#,#attributes.manager_non_id#,<cfif listlen(attributes.supervisor_id) GT 1>#listgetat(attributes.supervisor_id, 1)#<cfelseif listlen(attributes.supervisor_id) EQ 1>#attributes.supervisor_id#<cfelse>#variables.user_identification#</cfif><cfif len(attributes.photo)>,#attributes.photo#</cfif>,#attributes.employee_classification#,#createodbcdatetime(attributes.hire_date)#,#overtime#<cfif len(attributes.employee_type)>, #attributes.employee_type#</cfif>)
</cfquery><!--- 
