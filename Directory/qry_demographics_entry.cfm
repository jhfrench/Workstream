...
INSERT INTO Demographics_Ngauge (emp_id, ssn, hire_date,
	dob, manager_non_id, supervisor,
	employee_classification_id, effective_from, overtime_elligible_ind
	<cfif len(attributes.employee_type)>, employee_type_id</cfif><cfif len(attributes.photo)>, photo</cfif>)
VALUES (#variables.emp_id#, '#attributes.ssn#', #createodbcdatetime(attributes.hire_date)#,
	...
	#attributes.employee_classification#, #createodbcdatetime(attributes.hire_date)#, #attributes.overtime_elligible_ind#
	<cfif len(attributes.employee_type)>, #attributes.employee_type#</cfif><cfif len(attributes.photo)>,#attributes.photo#</cfif>)
</cfquery>
<!--- 
